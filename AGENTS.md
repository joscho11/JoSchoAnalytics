...
- The promoted QB-retaining Sunday-to-Tuesday market Ridge with injured-reserve starters has 44 core features and 47 fitted inputs. Its 2021-2025 walk-forward benchmark is 246/423 = 58.16% ATS, 11 pushes among 434 HIGH labels, Wilson lower 54.17%, graded at the best US Tuesday quote (`betting/market_model_benchmark_v3.json`; v2 is history). Five general injury features and vacated snaps were removed; four QB features remain; `ir_starters_diff` (home minus away non-QB starters on regular IR, from each team's previous-game roster) is the one added input. The Tuesday US median drives model input, pick, edge, and HIGH qualification; the best quote drives display, execution, and grading. A market move alone cannot add HIGH, but a validated model-version correction may change HIGH labels at the same frozen line. Do not move qualification onto the shopped quote. Never claim CLV.
