...
- **`app.py`** — **Thin multipage entrypoint, NOT a tab monolith.** The 8-tab app was migrated to page modules in site-revamp Batch 3. It builds **9 `st.Page` objects** and runs `st.navigation(..., position="top")` in three groups — **Betting:** Weekly Predictions (`default=True`), Track Record · **Fantasy:** 📋 Draft Board, 🧬 Rookie Board, Weekly Fantasy, DFS Optimizer · **More:** Film Room, League History, Help & Guide. Each `page_*.py` module is loaded through `_lazy_render` only when selected; do not restore eager page imports, which initialize every page's dependency graph on each cold process. Shared chrome/data live in `dashboard_chrome.py` / `dashboard_data.py` / `page_common.py`, and cross-links go through `nav_registry.PAGES`. `test_site_nav.py::test_every_page_renders_offline_clean` gives every page its own AppTest render, supplemented by the page-specific suites (`test_board_page.py`, `test_betting_pages.py`, `test_fantasy_league_pages.py`, `test_help_page.py`, `test_app_draft_board.py`, `test_app_talent_columns.py`, `test_page_rookie_board.py`). The pure, Streamlit-free helpers (`metric_card`, `get_confidence`, `_md_to_html`, `load_tracker`, `load_totals_tracker`) live in **`dashboard_utils.py`** (unit-tested by `test_dashboard_utils.py`, in CI). Game cards use the sportsbook-style display negation (see Key Constraints) and show a **dashed amber EXPERIMENTAL UNDER badge** when the totals model has a HIGH pick (amber because live 2025 is only at break-even). The Track Record totals section carries a "tracking only — do not bet" banner. **`APP_OFFLINE=1`** disables every network path (GA, nflreadpy, Sleeper) for hermetic AppTest runs. The Draft Board page (`page_draft_board.render` → `draft_board_2026.render()`) was **rebuilt 2026-07-22** into a season-projection comparison table (retired the frozen Phase-4 band) — READ-ONLY over the projection results + `season_dataset` ADP + talent artifacts, descriptive-only, no BUY/FADE/tier/verdict language. (The old inline Draft Value Finder tab and its `value_board_*.csv` were retired 2026-07-12; `build_value_board.py`/`build_draft_board.py`/`board_view.py` stay on disk but no page renders them.)
- **`model_explanations.py` / `model_explanations_snapshot.json`** — Help-page feature-influence cards are served from a small checked snapshot; the live app does not import `joblib`/NumPy or unpickle/hash 22 production artifacts. `test_model_explanations.py` verifies all recorded hashes against the current artifacts. After intentionally replacing a covered model, run `python scripts/build_model_explanations_snapshot.py`; changed Tree SHAP models require recomputing their pinned summaries first.
...
`.github/workflows/test.yml` runs on every push and PR against `main`, with **three** jobs: (1) **`features`** — `pytest betting/test_features.py` (the feature-pipeline contract tests, including the order-hash check that catches `PROD_FEATURES_35` / `FEATURE_COLS_85` reorder bugs) **plus `betting/test_calibration.py`**; (2) **`pytests`** — an explicit 13-file list of the seasonal / dashboard / talent suites (`test_dashboard_utils.py`, `test_seasonal_projections.py`, `test_draft_board.py`, `test_app_draft_board.py`, `test_site_nav.py`, `test_board_page.py`, `test_betting_pages.py`, `test_fantasy_league_pages.py`, `test_help_page.py`, `test_model_explanations.py`, `test_app_talent_columns.py`, `fantasy/talent/tests/test_talent_build.py`, `test_phase2.py`) via `requirements-test.txt`, **plus a second step running the six betting execution-layer suites from `working-directory: betting`**; (3) **`deploy-parity`** — the dashboard/runtime suites run on Python 3.12 against `requirements.txt` exactly as Streamlit Cloud resolves it; training/seasonal suites remain in the full-dependency jobs instead of bloating the cloud runtime. Explicit file lists, never auto-discovery, so research scripts named `*_test.py` are never collected. Enumerated 2026-07-28, **eight** test files appear in no job in `test.yml`: `test_nfl_qb_score.py`, `test_college_qb_score.py`, `test_board_refresh.py`, `test_page_rookie_board.py`, and the four `fantasy/projections/test_*.py` files. The two talent suites additionally need the talent artifacts committed before they can be wired in; the other six have no such blocker. Re-check the gap with: `for each test_*.py, grep its basename in .github/workflows/test.yml`.
...
the overlay; the committed overlay now carries all 245 rows. Tests: `test_board_page.py`,
...
`test_nfl_qb_score.py`) and is read only as a base layer, so a *missing* per-position file degrades to the old
...
- **Every build must be imported without running.** All eight expose `build()` behind an
  ...
  because the rebuild was byte-identical against a warm nflreadpy cache. `test_nfl_qb_score.py::test_importing_a_build_never_runs_it` now pins this.
...
**Not in CI (gap):** `test_nfl_qb_score.py` and `test_college_qb_score.py` are not in any `test.yml` job
list; they depend on the talent artifacts, which are untracked until committed. Add them once those land.
They are **not the only gap** — nine test files in total are absent from CI (see the Automation section).
