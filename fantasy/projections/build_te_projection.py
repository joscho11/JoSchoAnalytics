"""TE SEASON-TOTAL half-PPR PROJECTION — BUILD under committed prereg
PREREG_te_projection_2026-07-21.md.

REUSES the position-agnostic RB engine by IMPORT (build_rb_projection). Defines only TE-specific assembly:
a TE frozen-matrix twin (position=='TE' + pff_receiving — TEs are receivers, same block as WR) and the TE
feature pools. **build_rb_projection.py / build_wr_projection.py are NOT modified.** NO depth_rank in
either bucket. TE is the thinnest position (§3): small rookie folds, zero-heavy target — reported honestly.
...
TE_SCRATCH = Path(os.environ.get("TE_SCRATCH",
    r"C:/Users/josep/AppData/Local/Temp/claude/c--Users-josep-Desktop-random-stuff-cowork-OS/"
    r"a0db9953-ac7c-4009-99e9-0c070fb2e7da/scratchpad/te_projection"))
TE_SCRATCH.mkdir(parents=True, exist_ok=True)

# ---- TE feature pools (prereg §4) — NO depth_rank; rookie = WR receiving block (TEs are receivers) ----
TE_VET_ALL = list(VET_FEATS)                           # the same 32-col season_dataset pool
TE_CFB = ["cfb_final_dom", "cfb_best_dom", "cfb_scrim_ypg", "cfb_rec_ypg", "cfb_rec_pg", "cfb_ypr",
          "cfb_final_recshare", "cfb_career_scrim_yds", "cfb_career_scrim_td", "cfb_seasons",
          "cfb_breakout_class"]
TE_PFF = ["pff_receiving_grades_offense", "pff_receiving_grades_pass_route", "pff_receiving_yprr",
...
FROZEN_JOIN_TE = ROOK_COMBINE + TE_CFB + TE_PFF
TE_ROOK_ALL = ROOK_DRAFT + ROOK_AGE + ROOK_COMBINE + TE_CFB + TE_PFF + ROOK_LAND


def frozen_te_matrix():
    """TE twin of frozen_rb_matrix: regenerate the FROZEN hit-model matrix in a temp dir (PFF-derived
    parquet never touches the repo), return the TE slice keyed by gsis/norm_name."""
    scr = Path(tempfile.mkdtemp(prefix="te_frozen_"))
    ...
    frz = frz[frz["position"].astype(str) == "TE"].copy()
    missing = [c for c in FROZEN_JOIN_TE if c not in frz.columns]
    assert not missing, f"frozen TE matrix missing expected cols: {missing}"
    keep = ["gsis_id", "norm_name", "position", "entry_year"] + FROZEN_JOIN_TE
...
def assemble():
    ...
    te = sd[sd["position"] == "TE"].copy()
    te["log_pick"] = np.log(te["draft_pick"].clip(lower=1))
    tgt = season_total_target()
    te = te.merge(tgt, on=["player_id", "season"], how="left")
    pre = te["season"] <= MAXOBS
    te.loc[pre, "y"] = te.loc[pre, "y"].fillna(0.0)
    vet = te[te["is_rookie"] == 0].copy()
    rook = te[te["is_rookie"] == 1].copy()

    frz = frozen_te_matrix()
    ...
    need = rook[FROZEN_JOIN_TE].isna().all(axis=1)
...
def run_asserts(vet, rook):
    print("=" * 74); print("STEP 2 — TE PRE-REGISTERED ASSERTS (no model metric)"); print("=" * 74)
    ok = True
    ...
    talent_leak = [c for c in (TE_VET_ALL + TE_ROOK_ALL) if ("talent" in c or "efficiency" in c or c == "y")]
    # depth-CHART tokens only (NOT avg_depth_of_target, which is college aDOT)
    depth_leak = [c for c in (TE_VET_ALL + TE_ROOK_ALL) if ("depth_rank" in c or "depth_chart" in c or "depth_team" in c)]
    ...
    same = [c for c in TE_VET_ALL if not (c.startswith("prior_") or c in prior_derived or c in knowable)]
    ...
    feats = [c for c in TE_VET_ALL if v[c].notna().sum() >= 30 and v[c].nunique(dropna=True) >= 3]
    ...
    assert ok, "TE PRE-REGISTERED ASSERTS FAILED — STOP"
    print("\nSTEP 2 ASSERTS: PASS")


def coverage_report(vet, rook, te):
    print("\n--- coverage / structure ---")
    print(f"VETERAN: rows {len(vet)} | 2021-2025 {len(vet[vet.season.isin(TEST_SEASONS)])} | 2026 {len(vet[vet.season==DEPLOY])} | features {len(TE_VET_ALL)}")
    print(f"ROOKIE:  rows {len(rook)} | 2021-2025 {len(rook[rook.season.isin(TEST_SEASONS)])} | 2026 {len(rook[rook.season==DEPLOY])} | features {len(TE_ROOK_ALL)}")
    print("  ROOKIE test rows/fold (THIN — §3):", [int((rook.season == Y).sum()) for Y in TEST_SEASONS])
    g = te.groupby("season").apply(lambda d: pd.Series({
    ...
    print("\n2026 TE opportunity-feature coverage (provisional gap, prereg §4/§6):")
    r26 = te[te.season == DEPLOY]
...
def report_2026(vp, rp):
    print("\n" + "=" * 74); print("STEP 3 — 2026 TE PROJECTIONS (face-validity; not integrated)"); print("=" * 74)
    ...
    print("\nTOP-15 projected TEs (2026):"); print(show.head(15).to_string(index=False))
    print("\nTOP-12 ROOKIE TEs (2026):"); print(show[show.grp == "rook"].head(12).to_string(index=False))


def do_assemble():
    print("=" * 74); print("TE PROJECTION BUILD — ASSEMBLE (prereg PREREG_te_projection_2026-07-21.md)"); print("=" * 74)
    vet, rook, te = assemble()
    run_asserts(vet, rook)
    coverage_report(vet, rook, te)
    vet.to_parquet(TE_SCRATCH / "vet.parquet", index=False)
    rook.to_parquet(TE_SCRATCH / "rook.parquet", index=False)
    print(f"\nwrote {TE_SCRATCH/'vet.parquet'} ({len(vet)}) + rook.parquet ({len(rook)}) [scratch only]")


def do_walk_forward():
    vet = pd.read_parquet(TE_SCRATCH / "vet.parquet"); rook = pd.read_parquet(TE_SCRATCH / "rook.parquet")
    print("=" * 74); print("TE PROJECTION BUILD — WALK-FORWARD + SLEEPER + 2026"); print("=" * 74)
    print("\nVETERAN nested-CV walk-forward:")
    vout, _ = walk_forward(vet, TE_VET_ALL, "vet")
    print("\nROOKIE nested-CV walk-forward (THIN — §3):")
    rout, _ = walk_forward(rook, TE_ROOK_ALL, "rook")
    merged = pd.concat([vout.assign(grp="vet"), rout.assign(grp="rook")], ignore_index=True)
    merged.to_parquet(TE_SCRATCH / "walkforward_preds.parquet", index=False)
    ...
    vb = fit_final_model(vet, TE_VET_ALL); rb_ = fit_final_model(rook, TE_ROOK_ALL)
    ...
    pd.concat([v26.assign(grp="vet"), r26.assign(grp="rook")], ignore_index=True).to_parquet(
        TE_SCRATCH / "proj_2026.parquet", index=False)
    report_2026(v26, r26)
    print("\n" + "=" * 74); print("STOP 2 (HARD) — readout complete. Rookie-arm ship/hold is Joseph's call. Awaiting."); print("=" * 74)


def do_ship():
    import joblib
    ship_rookie = os.environ.get("TE_SHIP_ROOKIE", "1") == "1"    # Joseph's STOP-2 decision (default ship)
    print("=" * 74); print(f"TE PROJECTION BUILD — SHIP (rookie arm: {'SHIP' if ship_rookie else 'HOLD/coming'})"); print("=" * 74)
    vet = pd.read_parquet(TE_SCRATCH / "vet.parquet"); rook = pd.read_parquet(TE_SCRATCH / "rook.parquet")
    wf = pd.read_parquet(TE_SCRATCH / "walkforward_preds.parquet")
    vb = fit_final_model(vet, TE_VET_ALL); joblib.dump(vb, MODELS_DIR / "te_veteran_model.pkl")
    md5s = {"te_veteran_model.pkl": _md5(MODELS_DIR / "te_veteran_model.pkl")}
    print(f"  veteran deploy: {vb['family']} {vb['params']} (inner-MAE {vb['inner_cv_mae']:.3f})")
    rb_ = None
    if ship_rookie:
        rb_ = fit_final_model(rook, TE_ROOK_ALL); joblib.dump(rb_, MODELS_DIR / "te_rookie_model.pkl")
        md5s["te_rookie_model.pkl"] = _md5(MODELS_DIR / "te_rookie_model.pkl")
        print(f"  rookie  deploy: {rb_['family']} {rb_['params']} (inner-MAE {rb_['inner_cv_mae']:.3f})")
    else:
        print("  rookie  deploy: HELD (board shows 'coming' for TE rookies, like QB)")
    ...
    # board join file — ROOKIE-ONLY (the rookie board displays rookies; veterans live only in the full
    # te_projection_2026.csv, same as RB/WR). Empty if the rookie arm is HELD (TE rookies then show
    # "coming"). 2024/2025 = walk-forward OOS rookie preds; 2026 = deploy rookie preds.
    if ship_rookie:
        ...
        bp["norm_name"] = bp["player"].map(norm_name); bp["position"] = "TE"
    ...
    bp.to_csv(RESULTS_DIR / "te_rookie_board_projection.csv", index=False)

    wf_out = wf[["season", "grp", "player_id", "player", "y", "pred", "sleeper", "model"]].copy()
    wf_out["pred"] = np.round(wf_out["pred"], 1)
    wf_out.to_csv(RESULTS_DIR / "te_walkforward_predictions.csv", index=False)
    ...
    pd.DataFrame(rows).to_csv(RESULTS_DIR / "te_sleeper_comparison.csv", index=False)
    ...
    print(f"rookie arm on board: {'YES' if ship_rookie else 'NO (coming placeholder)'}")
    print(f"rookie_ppg_model.pkl md5 UNCHANGED: {ROOKIE_PPG_MD5}")
    print(f"\n2026 TE board projections written ({len(bp[bp.entry_class==2026])} class 2026 rows):")
    print(bp[bp.entry_class == 2026].sort_values("projection", ascending=False).head(15).to_string(index=False))
