...
## v3.9s — THE AUTHORIZED RUNNER (result ownership, adapter, CLI, 5-file mapping, 2026-08-03)

All in `tests/test_authorized_runner_v39.py`.

| # | requirement | generating function | covering test | status |
|---|---|---|---|---|
| R1-1 | results in `data/` fail the existing gate; results in `results/` leave it untouched; the input gate is UNCHANGED | `write_v39_results.RESULT_FILES`, `preflight` | `test_RED_result_files_in_the_DATA_dir_fail_the_existing_check`, `test_GREEN_results_in_the_RESULTS_dir_leave_the_data_check_untouched`, `test_the_input_artifact_gate_is_UNCHANGED`, `test_the_five_result_names_are_exactly_the_preregistered_ones` | PASS |
| R1-2 | exactly five names; missing, extra or empty refuses; pre-existing refuses without an explicit, separately tested overwrite flag | `validate_outputs`, `write_results` | `test_a_missing_frame_refuses`, `test_an_EXTRA_frame_refuses_rather_than_being_dropped`, `test_an_empty_frame_refuses`, `test_a_PRE_EXISTING_output_refuses_without_the_overwrite_flag`, `test_the_overwrite_flag_is_SEPARATELY_tested_and_works` | PASS |
| R1-3 | atomic write; a partial write fails closed and leaves nothing; hashes only after all five land | `write_results` | `test_a_PARTIAL_write_fails_closed_and_leaves_nothing`, `test_hashes_are_emitted_only_after_all_five_land` | PASS |
| R2-1 | the adapter joins ONLY on the frozen keys, one-to-one, preserving population and order, renaming the target to `y` only here | `panel_for_experiment` | `test_GREEN_the_adapter_produces_the_panel_run_experiment_needs`, `test_the_adapter_joins_ONLY_on_the_frozen_panel_keys`, `test_the_target_is_renamed_ONLY_at_this_boundary`, `test_the_adapter_is_INSENSITIVE_to_outcome_row_order_but_preserves_feature_order` | PASS |
| R2-2 | duplicate, missing and extra outcome keys refuse; a bad feature frame refuses | `panel_for_experiment` | `test_the_adapter_refuses_DUPLICATE_outcome_keys`, `test_the_adapter_refuses_a_MISSING_outcome_key`, `test_the_adapter_refuses_an_EXTRA_outcome_key`, `test_the_adapter_refuses_a_feature_frame_that_fails_its_validator` | PASS |
| R2-3 | zero-fill/accounting states retained and reported; no outcome field enters a model feature list | `panel_for_experiment`, `panel_feature_columns` | `test_the_adapter_retains_and_reports_the_accounting_states` | PASS |
| R2-4 | an incomplete panel REFUSES rather than letting `run_experiment` silently skip a bucket | `panel_bucket_gaps` | `test_the_bucket_coverage_guard_REFUSES_an_incomplete_panel`, `test_panel_bucket_gaps_names_every_unfeedable_bucket` | PASS |
| R3-1 | exactly one authorized-real path; the `--real` dead end is gone; `--run-mode` and `--outer-seasons` exist and validate | `main`, `parse_outer_seasons` | `test_the_dead_end_real_flag_is_GONE_and_there_is_ONE_authorized_path`, `test_outer_season_parsing` (3 params), `test_outer_seasons_outside_the_frozen_set_refuse` (4 params) | PASS |
| R3-2 | every closed/partial/synthetic state reaches ZERO readers, and the order is authorization → clearance → readers → panel → adapter → experiment → write | `run_authorized_real` | `test_every_closed_or_PARTIAL_lock_state_reaches_ZERO_readers` (3 params), `test_the_synthetic_run_mode_never_reaches_the_real_path`, `test_an_authorized_real_invocation_is_BLOCKED_while_the_locks_are_shut`, `test_the_authorized_path_order_is_authorization_then_clearance_then_readers` | PASS |
| R4-1 | the seven returned frames map losslessly onto five files, and oracle/preflight are recoverable EXACTLY from the serialized outputs | `compose`, `recover_oracle`, `recover_preflight` | `test_every_returned_frame_reaches_a_file`, `test_the_ORACLE_frame_is_recoverable_EXACTLY_from_the_serialized_metrics`, `test_the_METRICS_frame_is_recoverable_EXACTLY_from_the_serialized_metrics`, `test_the_PREFLIGHT_record_is_recoverable_EXACTLY_from_the_serialized_verdict`, `test_the_VERDICT_frame_is_recoverable_EXACTLY_from_the_serialized_verdict`, `test_no_oracle_or_preflight_FIELD_is_dropped`, `test_the_discriminators_are_explicit_and_present` | PASS |
| R6-1 | synthetic end-to-end through adapter and writer into a temp dir, writing nothing elsewhere; production and pinned inputs unchanged | — | `test_SYNTHETIC_end_to_end_adapter_then_experiment_then_writer`, `test_the_end_to_end_wrote_nothing_outside_the_temp_dir`, `test_production_models_and_pinned_inputs_are_unchanged`, `test_the_locks_are_still_shut_and_the_gate_still_refuses`, `test_no_real_result_file_exists_yet` | PASS |
| W1-1 | the timing check is PHASE-AWARE and disabled in neither phase: `pre_run` requires every counter EXACTLY zero and fails on a stale one; `post_pipeline` requires every frozen counter POSITIVE and NAMES the ones that did not execute | `preflight`, `validate_preflight_phase` | `test_pre_run_requires_every_counter_to_be_exactly_zero`, `test_a_STALE_nonzero_counter_FAILS_pre_run`, `test_post_pipeline_requires_every_counter_POSITIVE_and_NAMES_the_ones_that_did_not_run`, `test_ONE_missing_counter_fails_post_pipeline_and_is_named`, `test_a_boolean_counter_does_not_satisfy_post_pipeline`, `test_missing_unknown_or_malformed_phases_are_REFUSED` (10 params) | PASS |
| W1-2 | the check name is truthful in both phases and the frozen count stays 21, with both pinned vocabularies updated by value | `PREFLIGHT_CHECKS`, `FROZEN_AUTHORIZED_PREFLIGHT_CHECKS` | `test_the_check_is_named_truthfully_in_both_phases`, `test_BOTH_phases_evaluate_all_21_checks`, `test_the_result_carries_its_phase`, `test_the_phase_vocabulary_is_frozen_and_shared_by_value`, `test_frozen_pipeline_assertion_names` | PASS |
| W1-3 | a phase result cannot be replayed as the other gate, and a missing/unknown phase authorizes nothing | `validate_authorized_preflight`, `validate_post_pipeline_preflight` | `test_gate1_accepts_only_pre_run`, `test_gate2_accepts_only_post_pipeline`, `test_a_result_with_a_missing_or_unknown_phase_authorizes_NOTHING` (4 params), `test_post_pipeline_clearance_REFUSES_a_pre_run_result_and_writes_nothing` | PASS |
| W2-1 | the REAL authorized control flow clears both phases in order with NO injected counters and NO stubbed clearance, and the writer is reached only afterward | `run_authorized_real` | `test_the_REAL_authorized_control_flow_clears_BOTH_phases_and_only_then_writes`, `test_the_per_position_C10_records_use_post_pipeline_semantics` | PASS |
| W2-2 | ZERO result files survive any failure path — stale pre-run counters (before any reader), zero or missing post-pipeline counters, a wrong-phase result, or no capability | `run_authorized_real`, `require_post_pipeline_clearance` | `test_a_stale_counter_fails_pre_run_BEFORE_any_reader_and_writes_nothing`, `test_zero_counters_fail_post_pipeline_BEFORE_any_write`, `test_one_missing_counter_fails_post_pipeline_before_any_write`, `test_a_wrong_phase_result_refuses_the_run_and_writes_nothing`, `test_no_capability_refuses_before_anything` | PASS |
| W3-1 | authorized_real defaults to and accepts ONLY the frozen draw counts; the documented PowerShell command is pinned; reduced draws are test-only | `validate_authorized_draw_counts`, `main` | `test_the_frozen_draw_constants`, `test_authorized_real_accepts_ONLY_the_frozen_draw_counts` (9 cases), `test_the_CLI_defaults_to_the_frozen_counts_in_authorized_real_and_refuses_others`, `test_the_documented_powershell_command_is_pinned_and_carries_the_frozen_draws` | PASS |
| W3-2 | the phase repair moves NO statistical rule and leaves the source lock closed | frozen constants | `test_no_frozen_statistical_constant_moved`, `test_the_source_lock_constant_is_still_closed` | PASS |
| X1-1 | ONE canonical panel-key contract (`player_id` StringDtype(python), `season` int32, keys non-null and unique), validated BEFORE conversion and never a blind cast | `PANEL_KEY_DTYPES`, `canonicalize_panel_keys` | `test_the_canonical_key_dtypes_are_frozen`, `test_canonicalization_produces_the_contract_from_the_object_shape`, `test_canonicalization_is_idempotent_and_preserves_values_and_order`, `test_the_module_does_not_blindly_cast_the_player_key` | PASS |
| X1-2 | null, stringified-null and non-textual player ids and bad seasons are REFUSED, not coerced | `canonicalize_panel_keys` | `test_null_and_stringified_null_player_ids_are_REFUSED` (7 params), `test_numeric_player_ids_are_REFUSED_not_stringified` (3 params), `test_bad_seasons_are_REFUSED` (5 params), `test_null_seasons_are_REFUSED` (2 params), `test_nonnumeric_seasons_are_REFUSED`, `test_an_integral_float_season_IS_accepted_and_narrows_losslessly`, `test_duplicate_keys_are_REFUSED`, `test_a_missing_key_column_is_REFUSED` | PASS |
| X2-1 | both real reader boundaries emit the canonical dtypes, and an injected noncanonical frame refuses with an accurate key-dtype error | `authorized_composed_feature_reader`, `grouped_season_totals`, `assemble_panel_core` | `test_the_grouped_outcome_reader_emits_the_canonical_key_dtypes`, `test_the_outcome_reader_REG_filter_and_formula_are_unchanged`, `test_a_noncanonical_injected_frame_fails_on_DTYPE_not_on_ordering`, `test_canonical_reader_output_shapes_pass_assemble_panel_core` | PASS |
| X3-1 | the adapter separates dtype from ordering: the real failure shape passes, dtype violations report as dtype, genuine reordering still reports as reordering with the first moved position | `panel_for_experiment`, `ordered_key_values` | `test_THE_REAL_FAILURE_SHAPE_string_features_vs_object_outcomes_is_reproduced_and_fixed`, `test_the_adapter_reports_a_dtype_violation_as_a_dtype_violation`, `test_ACTUAL_reordering_is_still_reported_as_reordering`, `test_the_reordering_message_names_the_first_moved_position`, `test_the_ordering_comparison_ignores_dtype_and_only_sees_moved_VALUES`, `test_missing_extra_and_duplicate_keys_still_fail_in_the_adapter` | PASS |
| X4-1 | synthetic weekly + feature parquets reach an aligned panel through the REAL reader/core/adapter path, without reading the canonical outcome snapshot | `grouped_season_totals`, `assemble_panel_core` | `test_reader_to_core_to_adapter_reaches_an_aligned_panel_without_the_real_snapshot`, `test_the_canonical_weekly_snapshot_is_NOT_read_by_this_module` | PASS |
