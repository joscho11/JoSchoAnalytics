"""TE season-total projection harness (prereg PREREG_te_projection_2026-07-21.md).

NEW file — does NOT modify rb_projection_harness.py or wr_projection_harness.py. Mirrors the proof on TE
data (the thinnest position — small rookie folds, zero-heavy target; §3 of the prereg).

--build : SYNTHETIC/STRUCTURAL proof ONLY. NO real fit, NO real accuracy, NO Sleeper join.
          Proves routing partitions exhaustively; leakage guard (aligned prior feature carries signal,
          shuffled alignment destroys it, planted leak screams); walk-forward never trains on the test
          season; planted signal detected & pure noise not; nested-CV selects without touching the outer
          test fold; F-step full pipeline on real TE shapes with a PERMUTED target yields valid shapes.
          NO depth-chart feature (carry-forward lesson). If a rookie fold is too thin to run, it is
          reported honestly, not forced. The real fit is a LATER session.
"""
import sys, argparse
...
def nested_select(train_df, feats, y_col, params_grid):
    ...
    for params in params_grid:
        ...
        for s in seasons:
            ...
            if len(itr) < 40 or len(iva) < 8:            # thinner floor for TE (§3), still guards blindness
                continue
...
def walk_forward(df, feats, y_col, tune=False):
    ...
    for Y in TEST_SEASONS:
        ...
        if len(tr) < 40 or len(te) == 0:                 # thin-position floor; report skips honestly
            print(f"    (skipped fold {Y}: train {len(tr)} test {len(te)} too thin)")
            continue
        B.assert_walk_forward_fold(tr, te, Y, "te_harness", pool=df, y=y_col)
...
def prove():
    print("=" * 70); print("TE PROJECTION HARNESS — SYNTHETIC/STRUCTURAL PROOF (no real fit, no real metric)")
    print("=" * 70)
    rng = np.random.default_rng(SEED)
    df = load_te(); df = df[df.season <= 2025].copy()
    vet, rk = route(df)
    ...
    print(f"   [TE thinness: rookie test rows/fold = "
    ...
    noise_rank = _rank(*(lambda o: (o.y, o.p))(walk_forward(v, VET_FEATS, "y_noise")))
    ...
    try:
        _rep = B.assert_walk_forward_folds(v, "te_harness", y="y_plant")
        folds_ok = True
...
def real_pathproof():
    print("\n" + "=" * 70); print("F-STEP — FULL PIPELINE ON REAL TE SHAPES, PERMUTED TARGET (no real metric)")
    print("=" * 70)
    rng = np.random.default_rng(SEED)
    df = load_te(); df = df[df.season <= 2025].copy()
    vet, rk = route(df)
    ...
    print(f"\nF-STEP: {'PASS — full pipeline yields valid-shaped projections on real TE data; NO real fit/metric.' if allok else 'FAIL'}")
    return allok
