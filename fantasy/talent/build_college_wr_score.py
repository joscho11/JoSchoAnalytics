"""Build the College WR Talent Score — SPEC R38 (RATIFIED-2026-07-27).

Emits `college_wr_score_2026.csv`. gamma = 0.5 (57/29/14). Weights sum 1.000.
`yardshare`, `dominator` and any target-share term are DELIBERATELY ABSENT (opportunity confounds).

Display anchored on WRs who REACHED THE NFL — ruled for WR on its own terms, not inherited:
drafted-only puts 4 of 7 anchors in band vs 3 for the all-WR pool, and the pool is 755 of 2,872
so the small-pool objection that applied at QB does not apply here.

FRAMING (carried from Joseph): WR is DEAD across six instrument classes. These anchors test only
whether the score DESCRIBES college WR talent. A high score for a future bust is the construct
working, not failing.

TWO DISCLOSED DEFECTS, shipped knowingly:
  1. `contested_catch_rate` delivers only .055 EFFECTIVE against .100 nominal (r = 0.152 on a
     median 17 contested targets). Raising it to .180 fixes the mechanics (.104 effective) and
     KNOCKS Drake London — the archetype the facet exists for — out of band. Measured, rejected.
  2. The missing-contested omit-and-rescale path affects **709 of 2,872 players (228 of 755
     drafted, ~30%)**, not the ~87 the spec anticipated. Roughly a third of the drafted pool is
     scored on five facets rather than six, so the composite's facet set varies by row.

SUBSTITUTION: no `catchable_targets` column exists; `grades_hands_drop` uses `targets`.
Travis Hunter is UNSCORABLE at the 150-route floor (two-way snaps).

Run:  python fantasy/talent/build_college_wr_score.py
...
OUT = HERE / "college_wr_score_2026.csv"
PROV = HERE / "college_wr_score_2026.provenance.json"
sys.path.insert(0, str(SEAS))
from _utils import norm_name  # noqa: E402
GAMMA, ROUTE_FLOOR, YEARS = 0.5, 150, range(2014, 2026)
ANCHOR = dict(lo_pct=5, hi_pct=98, lo_score=52, hi_score=95, clip=(50, 99))

W = {"grades_pass_route": .400, "yprr": .250, "contested_catch_rate": .100,
     "avoided_tackles_rec": .100, "grades_hands_drop": .075, "yac_per_reception": .075}
...
def build():
    ...
    print(f"college WR player-seasons (>= {ROUTE_FLOOR} routes, 2014-2025): {len(panel):,}")
    ...
    for pid, g in panel.groupby("player_id"):
        ...
        row["college_wr"] = num / den
        rows.append(row)

    C = pd.DataFrame(rows)
    print(f"\nscored {len(C):,} WRs | rescaled for missing contested: "
    ...
    print(f"\npool sizes — all WRs {len(C):,} | reached NFL {int(C.drafted.sum()):,}")

    for tag, mask in (("DRAFTED-ONLY", C.drafted), ("ALL WRs", pd.Series(True, index=C.index))):
        sl = C.loc[mask, "college_wr"]
        z2, z98 = np.percentile(sl, [ANCHOR["lo_pct"], ANCHOR["hi_pct"]])
        b = (ANCHOR["hi_score"] - ANCHOR["lo_score"]) / (z98 - z2)
        a = ANCHOR["lo_score"] - b * z2
        C[f"score_{tag}"] = (a + b * C.college_wr).clip(*ANCHOR["clip"]).round(1)
    ...
    ds = pd.read_csv(DATASET, usecols=["player_id", "player", "season", "position", "is_rookie"],
                     low_memory=False)
    rk = ds[(ds.season == 2026) & (ds.position == "WR") & (ds.is_rookie == 1)].copy()
    ...
    cols = (["player_id", "gsis_id", "nfl_player_id", "player", "norm_name", "college",
             ...
             "college_wr", "score", "rank_final_season"]
    ...
    PROV.write_text(json.dumps({
        "spec": "R38 (RATIFIED-2026-07-27)", "built": "2026-07-27",
        ...
        "anchor": {**ANCHOR, "fitted_on": "WRs who reached the NFL",
                   "n": int(C.drafted.sum()), "ruled_for_wr": "4/7 in band vs 3/7 for all-WR pool"},
        ...
        "known_defects": ["contested_catch_rate delivers .055 effective vs .100 nominal",
                          "~30% of the drafted pool is scored on 5 facets, not 6"],
        "framing": "WR is DEAD across six instrument classes; descriptive only",
        "md5": hashlib.md5(OUT.read_bytes()).hexdigest(),
    }, indent=2), encoding="utf-8")
    print(f"wrote {OUT.name}: {len(C):,} WRs ({int(C.drafted.sum())} reached NFL, "
